...
### デスクトップアプリはバックグラウンド常駐が必要

「最初にデスクトップ版でComputer Historyをオンにした後は、アプリを使わずCLIだけ起動すればよい」というわけではありません。CLIは生成済みのローカルメモリを利用できますが、新しい操作イベントを収集する役割はデスクトップアプリ側にあります。

OpenAIの公式ドキュメントは、アプリを完全終了した際の動作を明記していません。ただし、収集状態の確認、一時停止、再開はmacOSのメニューバーにあるChatGPTアイコンから操作する仕様です。この構成上、実用的にはChatGPTデスクトップアプリのプロセスをバックグラウンドで動かしておく必要があります。

macOSでは、ウィンドウを閉じることとアプリを完全終了することは別です。ウィンドウを閉じてもメニューバーにChatGPTが残っていれば、バックグラウンド動作は継続します。一方、`Quit`で完全終了した状態では、CLIが代わりに別アプリの操作を収集することはありません。すでに作られたメモリは残りますが、新しいComputer Historyは増えないと考えるのが安全です。

:::warning
デスクトップアプリのCPU・メモリ使用量やバッテリー消費が気になる人にとって、常駐が事実上必要な点はComputer Historyの欠点です。負荷を避けてCLIだけで運用する場合は、PC全体の履歴ではなく、Codex CLI自身のローカルMemoriesに限定されます。
:::
